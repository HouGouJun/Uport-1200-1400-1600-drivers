...
static int  mxuport_open(struct usb_serial_port *port, struct file *filp)
{
	...
	printk(KERN_EMERG "mxuport open.............\n");
	...
	if (port->tty)
		port->tty->port->low_latency = (mx_port->flags & ASYNC_LOW_LATENCY) ? 1 : 0;
...
static int mxuport_tiocmset (struct usb_serial_port *port, unsigned int set, unsigned int clear)
{
	int  status = 0;	
	unsigned int mcr;
	struct mxuport_port *mx_port = usb_get_serial_port_data(port);
...
static int mxuport_tiocmget(struct usb_serial_port *port)
{
	struct mxuport_port *mx_port;
...
static int mxuport_ioctl (struct usb_serial_port *port, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
        ...
        case TIOCMBIS:
	    case TIOCMBIC: 
			return mx_set_modem_info(port, cmd, arg);
...
static void mxuport_unthrottle (struct usb_serial_port *port)
{
    ...
    if((data_len > 0) && (!atomic_read(&mx_port->read_in_progress))){
		mx_port->port->tty->port->low_latency = 0;
        mx_tty_recv(mx_port->port->tty, mx_port);
		mx_port->port->tty->port->low_latency = (mx_port->flags & ASYNC_LOW_LATENCY) ? 1 : 0;
    }
...
static int mxuport_write(struct usb_serial_port *port, const unsigned char *data, int count)
{
	...
	printk(KERN_EMERG " mxuport write.......\n");
   ...
   unsigned long flags;	 	
	...
	do {
        if(mxport->hold_reason & MX_WAIT_FOR_UNTHROTTLE)
  	    ...
  	    room_cnt = tty_buffer_request_room(tty->port, length);
		if (room_cnt < length) {
		...
		tty_insert_flip_string(tty->port, rcv_buf, rx_cnt);
...
static int mx_set_modem_info(struct usb_serial_port *port, unsigned int cmd, unsigned long arg)
{
    int status;
	...
	status = mxuport_tiocmset(port, set, clear);
							  ...
							  struct serial_struct __user *new_arg)
{
     ...
     */
	if (port->tty)
        port->tty->port->low_latency = (mxport->flags & ASYNC_LOW_LATENCY) ? 1 : 0;
