...
#define err(format, arg...)  printk(KERN_ERR     "mxusbserial error: " format "\n" , ## arg)
#define info(format, arg...) printk(KERN_INFO    "mxusbserial error: " format "\n" , ## arg)
#define warn(format, arg...) printk(KERN_WARNING "mxusbserial error: " format "\n" , ## arg)
...
static struct tty_port *tport = NULL;
...
static const struct tty_port_operations uport_port_ops = {
};
...
int usb_serial_init(void)
{
	...
	tport = (struct tty_port*)kzalloc(SERIAL_TTY_MINORS * sizeof(struct tty_port), GFP_KERNEL);
	if(!tport)
		return -ENOMEM;
	...
	for (i = 0; i < SERIAL_TTY_MINORS; ++i) {
		...
		tport[i].ops = &uport_port_ops;
		tty_port_link_device(&tport[i], mx_usbserial_tty_driver, i);
	}	
